...
static u16 mv88e6xxx_port_vlan(struct mv88e6xxx_chip *chip, int dev, int port)
{
	...
	/* Frames from external DSA and CPU ports can egress any local port */
	...
	/* Frames from external user ports can egress any local DSA and CPU
	 * ports, as well as any local member of their bridge group.
	 */
...
static int mv88e6xxx_pvt_map(struct mv88e6xxx_chip *chip, int dev, int port)
{
	...
	if (!chip->info->ops->pvt_write)
		return -EOPNOTSUPP;

	/* The cross-chip Port VLAN is masked with the in-chip Port VLAN */
	if (chip->ds->index != dev)
		pvlan = mv88e6xxx_port_vlan(chip, dev, port);

	return chip->info->ops->pvt_write(chip, dev, port, pvlan);
}

static int mv88e6xxx_pvt_init(struct mv88e6xxx_chip *chip)
{
	int dev, port;
	int err;
...
static int mv88e6xxx_inchip_bridge_map(struct mv88e6xxx_chip *chip,
				       struct net_device *br)
{
	int i, err;
	...
	for (i = 0; i < mv88e6xxx_num_ports(chip); ++i) {
		if (chip->ds->ports[i].bridge_dev == br) {
			err = mv88e6xxx_port_vlan_map(chip, i);
			if (err)
				return err;
		}
	}
				      ...
				      struct net_device *br)
{
	...
	mutex_lock(&chip->reg_lock);
	err = mv88e6xxx_inchip_bridge_map(chip, br);
	mutex_unlock(&chip->reg_lock);
					...
					struct net_device *br)
{
	...
	mutex_lock(&chip->reg_lock);
	if (mv88e6xxx_inchip_bridge_map(chip, br) ||
...
static int mv88e6xxx_setup(struct dsa_switch *ds)
{
	...
	/* Initialize the Cross-chip Port VLAN Table */
	err = mv88e6xxx_pvt_init(chip);
	if (err)
		goto unlock;
...
static const struct mv88e6xxx_ops mv88e6085_ops = {
	...
	.pvt_read = mv88e6xxx_g2_pvt_read,
	.pvt_write = mv88e6xxx_g2_pvt_write,
