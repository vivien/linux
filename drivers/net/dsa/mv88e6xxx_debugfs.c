...
static int mv88e6xxx_8021q_mode_show(struct seq_file *s, void *p)
{
	...
	for (i = 0; i < ps->num_ports; ++i) {
		...
		switch (ret & PORT_CONTROL_2_8021Q_MASK) {
		case PORT_CONTROL_2_8021Q_DISABLED:
			seq_puts(s, " DISABLED\n");
			break;
		case PORT_CONTROL_2_8021Q_FALLBACK:
			seq_puts(s, " FALLBACK\n");
			break;
		case PORT_CONTROL_2_8021Q_CHECK:
			seq_puts(s, " CHECK\n");
			break;
		case PORT_CONTROL_2_8021Q_SECURE:
			seq_puts(s, " SECURE\n");
			break;
		default:
			seq_puts(s, " ???\n");
...
static void mv88e6xxx_init_debugfs(struct dsa_switch *ds)
{
	...
	debugfs_create_file("8021q_mode", S_IRUGO | S_IWUSR, ps->dbgfs, ds,
			    &mv88e6xxx_8021q_mode_fops);

	debugfs_create_file("vlan_table", S_IRUGO | S_IWUSR, ps->dbgfs, ds,
