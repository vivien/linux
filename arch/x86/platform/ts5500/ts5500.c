...
static struct resource ts5500_dio_lcd_resource[] = {
	DEFINE_RES_IRQ_NAMED(1, "LCD interrupt"),
};

static struct platform_device ts5500_dio_lcd_pdev = {
	.name = "ts5500-dio-lcd",
	.id = -1,
	.resource = ts5500_dio_lcd_resource,
	.num_resources = 1,
};
...
static int __init ts5500_init(void)
{
	...
	ts5500_dio_lcd_pdev.dev.parent = &pdev->dev;
	if (platform_device_register(&ts5500_dio_lcd_pdev))
		dev_warn(&pdev->dev, "LCD as DIO block registration failed\n");
